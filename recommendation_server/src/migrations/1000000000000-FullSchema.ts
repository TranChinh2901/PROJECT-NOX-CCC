...
export class FullSchema1000000000000 implements MigrationInterface {
  name = 'FullSchema1000000000000';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      CREATE TABLE \`order_status_histories\` (
        ...
        CONSTRAINT \`FK_order_status_histories_changed_by_user\` FOREIGN KEY (\`changed_by_user_id\`) REFERENCES \`users\`(\`id\`) ON DELETE SET NULL
      ) ENGINE=InnoDB;
    `);
    ...
    await queryRunner.query(`
      CREATE TABLE \`recommendation_cache\` (
        ...
        \`updated_at\` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        PRIMARY KEY (\`id\`),
        UNIQUE KEY \`UQ_recommendation_cache_user_product_type\` (\`user_id\`, \`product_id\`, \`recommendation_type\`),
